...
- [PIM Server Access CA Policy](../../policies/conditional-access/pim-server-access/README.md) — CA policy enforcing phishing-resistant MFA for Azure Management and PIM role activation.
- [Windows Hello for Business Policy](../../policies/conditional-access/windows-hello/README.md) — Intune policy configuring WHfB on managed Windows devices used for server access.
- [FIDO2 Security Key Policy](../../policies/conditional-access/fido2-security-key/README.md) — Entra ID policy for FIDO2 key registration (alternative to WHfB for server sign-in).
- [Step-by-step test plan](pim-jit-test-plan.md) — Numbered steps for validating PIM JIT access on an Arc server.
